...
\newpage % TODO: Al final, ver donde ponemos de estos
...
% Ideas:
% Tecnologias que no funcionaron: Akka, lo mitigamos adaptando la implementacion usando RabbitMQ, docker-compose (no estaba instalado en el servidr de FaMAF), lo solucionamos usando docker swarm, y de paso aprendimos a usarlo, sockets de zeromq, despliegue en redes neuronales no fue tan lineal, tambien hubo problemas en las versiones de python, en criptografia usamos un lenguaje muy nuevo (zig) que no tiene version estable, por lo que habia features que no funcionaban o funcionaban mal
% TUPAC no funciono porque era dificil de usar con los diferentes lenguajes, nos pasamos a FaMAF
% Temas de organizacion: al principio coordinamos las tareas pendientes con Trello, pero despues nos resulto mas facil la coordinacion "por whatsapp"
% Cambio de calendario: al inicio del trabajo profesional creiamos que ibamos a tener tiempo hasta finales de junio para terminarlo. A mediados del segundo cuatrimestre se nos informo que el trabajo se entregaba la segunda semana de junio. No fue tan grave porque ese mes lo teniamos reservado para ajustes e imprevistos.
